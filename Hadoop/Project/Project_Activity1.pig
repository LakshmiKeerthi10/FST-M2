-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/lakshmi/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/lakshmi/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/lakshmi/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
STORE namesOrdered INTO 'hdfs:///user/lakshmi/outputs' USING PigStorage('\t');
